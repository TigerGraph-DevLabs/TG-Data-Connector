...
public class BufferedRecordTransformerExchanger extends TransformerExchanger implements RecordSender, RecordReceiver {
    ...
    public void sendToWriter(Record record) {
        ...
        Validate.notNull(record, "Record cannot be empty.");
        ...
        if (record.getMemorySize() > this.byteCapacity) {
            this.pluginCollector.collectDirtyRecord(record, new Exception(String.format("A single record exceeded the size limit, the current limit is :%s", this.byteCapacity)));
            return;
        }
