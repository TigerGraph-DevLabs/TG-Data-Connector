...
public class JarLoader extends URLClassLoader {
    ...
    private static URL[] getURLs(String[] paths) {
        Validate.isTrue(null != paths && 0 != paths.length,
                "The JAR package path cannot be empty.");
    ...
    private static List<URL> doGetURLs(final String path) {
        Validate.isTrue(!StringUtils.isBlank(path), "The JAR package path cannot be empty.");

        File jarPath = new File(path);

        Validate.isTrue(jarPath.exists() && jarPath.isDirectory(),
                "The JAR package path must exist and be a directory.");
        ...
        for (int i = 0; i < allJars.length; i++) {
            try {
                ...
                throw DataXException.asDataXException(
                        ...
                        "System error loading JAR package", e);
            }
        }
