...
public class JobContainer extends AbstractContainer {
    ...
    public void start() {
        ...
        try {
            ...
            if(isDryRun) {
                ...
                // split 的作用
                // 1.将reader的config读进来
                // 2.根据reader的配置个数 设置writer的配置个数
                // 3.将reader writer transformer合并起来
                // 但是 它并没有开始执行 只是一个准备。
    ...
    private int split() {
        ...
        // 这是合并的 将reader的config + writer的config + transform的config 一起合并
        // key是CoreConstant.DATAX_JOB_CONTENT = "job.content"
    ...
    private void adjustChannelNumber() {
        ...
        if (isByteLimit) {
            ...
            if (channelLimitedByteSpeed == null || channelLimitedByteSpeed <= 0) {
                throw DataXException.asDataXException(
                        FrameworkErrorCode.CONFIG_ERROR,
                        "With a total BPS speed limit, the BPS value of a single channel cannot be null or non-positive");
            }
        ...
        if (isRecordLimit) {
            ...
            if (channelLimitedRecordSpeed == null || channelLimitedRecordSpeed <= 0) {
                throw DataXException.asDataXException(FrameworkErrorCode.CONFIG_ERROR,
                        "With a total TPS speed limit, the TPS value of a single channel cannot be null or non-positive");
            }
    ...
    private void schedule() {
        ...
        try {
            ...
            if (executeMode == ExecuteMode.LOCAL || executeMode == ExecuteMode.DISTRIBUTE) {
                if (this.jobId <= 0) {
                    throw DataXException.asDataXException(FrameworkErrorCode.RUNTIME_ERROR,
                            "In [local | distribute] mode must be set jobId, and its value > 0.");
                }
            }
            ...
            LOG.error("Run the Scheduler mode [{}] error.", executeMode);
            this.endTransferTimeStamp = System.currentTimeMillis();
            throw DataXException.asDataXException(
                    FrameworkErrorCode.RUNTIME_ERROR, e);
        }
    ...
    private List<Configuration> doReaderSplit(int adviceNumber) {
        ...
        if (readerSlicesConfigs == null || readerSlicesConfigs.size() <= 0) {
            throw DataXException.asDataXException(
                    ...
                    "The number of tasks that Reader splits cannot be less than or equal to zero");
        }
    ...
    private List<Configuration> doWriterSplit(int readerTaskNumber) {
        ...
        if (writerSlicesConfigs == null || writerSlicesConfigs.size() <= 0) {
            throw DataXException.asDataXException(
                    FrameworkErrorCode.PLUGIN_SPLIT_ERROR,
                    "Writer shards tasks that cannot be less than or equal to 0");
        }
    ...
    private List<Configuration> mergeReaderAndWriterTaskConfigs(
        ...
        if (readerTasksConfigs.size() != writerTasksConfigs.size()) {
            throw DataXException.asDataXException(
                    FrameworkErrorCode.PLUGIN_SPLIT_ERROR,
                    String.format("The number of tasks that Reader splits [%d] does not equal the number of tasks that Writer splits [%d].",
    ...
    private List<Configuration> distributeTasksToTaskGroup(
        ...
        Validate.isTrue(averTaskPerChannel > 0 && channelNumber > 0
                ...
                "The average number of tasks for each channel, the number of channels, and the average number of channels for each taskGroup should be positive");
