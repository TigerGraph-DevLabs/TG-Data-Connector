...
public final class ConfigParser {
    ...
    public static Configuration parse(final String jobPath) {
        ...
        try {
            ...
            LOG.warn(String.format("Plugin [%s,%s] failed to load, retry after 1s... Exception:%s", readerPluginName, writerPluginName, e.getMessage()));
            try {
                Thread.sleep(1000);
    ...
    private static String getJobContent(String jobResource) {
        ...
        if (isJobResourceFromHttp) {
            ...
            try {
                ...
                throw DataXException.asDataXException(FrameworkErrorCode.CONFIG_ERROR, "Failed to obtain job configuration information:" + jobResource, e);
            }
        } else {
        ...
        if (jobContent == null) {
            throw DataXException.asDataXException(FrameworkErrorCode.CONFIG_ERROR, "Failed to get job configuration information:" + jobResource);
        }
        return jobContent;
    }
    ...
    public static Configuration parsePluginConfig(List<String> wantPluginNames) {
        ...
        if (wantPluginNames != null && wantPluginNames.size() > 0 && wantPluginNames.size() != complete) {
            throw DataXException.asDataXException(FrameworkErrorCode.PLUGIN_INIT_ERROR, "Plugin load failed, the specified plug-in load was not completed:" + wantPluginNames);
        }
    ...
    public static Configuration parseOnePluginConfig(final String path,
        ...
        if(!pluginSet.contains(pluginName)) {
            ...
            throw DataXException.asDataXException(FrameworkErrorCode.PLUGIN_INIT_ERROR, "Plug-in load failed, duplicate plugins exist:" + filePath);
        }
