...
public final class RetryUtil {
    ...
    private static class Retry {
        ...
        public <T> T doRetry(Callable<T> callable, int retryTimes, long sleepTimeInMilliSecond, boolean exponential, List<Class<?>> retryExceptionClasss)
            ...
            if (null == callable) {
                throw new IllegalArgumentException("System programming error, Callable input parameter cannot be empty!");
            }

            if (retryTimes < 1) {
                throw new IllegalArgumentException(String.format(
                        "Error in system programming, input retryTime [%d] cannot be less than 1!", retryTimes));
            }
            ...
            for (int i = 0; i < retryTimes; i++) {
                try {
                    ...
                    saveException = e;
                    if (i == 0) {
                        LOG.error(String.format("Exception when calling callable, error Msg:%s", saveException.getMessage()), saveException);
                    }
                    ...
                    if (i + 1 < retryTimes && sleepTimeInMilliSecond > 0) {
                        ...
                        LOG.error(String.format("Exception when calling callable, About to attempt the %s retry.This retry planned to wait for [%s]ms, actual wait for [%s]ms, exception Msg:[%s]",
