...
public final class RangeSplitUtil {
    ...
    public static BigInteger[] doBigIntegerSplit(BigInteger left, BigInteger right, int expectSliceNumber) {
        if (expectSliceNumber < 1) {
            throw new IllegalArgumentException(String.format(
                    "The number of split copies should not be less than 1. Here:expectSliceNumber=[%s].", expectSliceNumber));
        }

        if (null == left || null == right) {
            throw new IllegalArgumentException(String.format(
                    "When sharding a BigInteger, the left and right ranges cannot be null.Here: left = [% s], right = [% s].", left, right));
        }
    ...
    public static BigInteger stringToBigInteger(String aString, int radix) {
        if (null == aString) {
            throw new IllegalArgumentException("The BigInteger parameter cannot be empty.");
        }
        ...
        for (int i = aString.length() - 1; i >= 0; i--) {
            ...
            if (tempChar >= 128) {
                throw new IllegalArgumentException(String.format("Shard by string only supports ASCII strings, whereas strings :[%s] non-ASCII strings.", aString));
            }
