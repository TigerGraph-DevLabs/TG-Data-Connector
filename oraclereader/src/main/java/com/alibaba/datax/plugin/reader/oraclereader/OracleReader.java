    ...
    public static class Job extends Reader.Job {
            ...
            if (fetchSize < 1) {
                throw DataXException
                        .asDataXException(DBUtilErrorCode.REQUIRED_VALUE,
                                String.format("The fetchSize you configured is incorrect. The fetchSize:[%d] value cannot be less than 1.",
                                        fetchSize));
            }
            originalConfig.set(
                    com.alibaba.datax.plugin.rdbms.reader.Constant.FETCH_SIZE,
        ...
        private void dealHint(Configuration originalConfig) {
            ...
            if (StringUtils.isNotBlank(hint)) {
                ...
                if (!isTableMode) {
                    throw DataXException.asDataXException(OracleReaderErrorCode.HINT_ERROR, "Hints can be configured only when a non-QuerySQL mode reads Oracle.");
                }
