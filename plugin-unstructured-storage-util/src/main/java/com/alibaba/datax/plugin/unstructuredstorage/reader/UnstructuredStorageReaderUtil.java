...
public class UnstructuredStorageReaderUtil {
	...
	public static void readFromStream(InputStream inputStream, String context,
		...
		try {
				...
				if ("lzo_deflate".equalsIgnoreCase(compress)) {
					...
					throw DataXException
							.asDataXException(
									...
									String.format("only support gzip, bzip2, zip, lzo, lzo_deflate, hadoop-snappy, framing-snappy" +
											"File compression format, the file compression format you configured is not supported: [%s]", compress));
				}
			}
			...
			throw DataXException.asDataXException(
					UnstructuredStorageReaderErrorCode.READ_FILE_IO_ERROR,
					String.format("Stream read error : [%s]", context), e);
		} finally {
	...
	public static void doReadFromStream(BufferedReader reader, String context,
		...
		try {
			...
			throw DataXException.asDataXException(
					...
					String.format("File not found: [%s]", context), fnfe);
		} catch (IOException ioe) {
			throw DataXException.asDataXException(
					UnstructuredStorageReaderErrorCode.READ_FILE_IO_ERROR,
					String.format("Error reading file: [%s]", context), ioe);
		} catch (Exception e) {
			throw DataXException.asDataXException(
					UnstructuredStorageReaderErrorCode.RUNTIME_EXCEPTION,
					...
					if (null == columnIndex && null == columnConst) {
						throw DataXException
								.asDataXException(
										...
										"Since you have configured Type, you need to configure at least Index or Value");
					}
					...
					if (null != columnIndex && null != columnConst) {
						throw DataXException
								.asDataXException(
										...
										"You have a mixture of index, value, and only one of each column can be selected at a time");
					}

					if (null != columnIndex) {
						if (columnIndex >= sourceLine.length) {
							String message = String
									.format("You are trying to read the column [%s] out of bounds. You are trying to read the column [%s].",
					...
					switch (type) {
						...
						case LONG:
							try {
								...
								throw new IllegalArgumentException(String.format(
										"Type conversion error, cannot convert [%s] to [%s]", columnValue,
										"LONG"));
							}
							break;
						case DOUBLE:
						...
						default:
							String errorMessage = String.format(
									"The column type you configured is not currently supported: [%s]", columnType);
	...
	public static void validateEncoding(Configuration readerConfiguration) {
		...
		try {
			...
			throw DataXException.asDataXException(UnstructuredStorageReaderErrorCode.ILLEGAL_VALUE,
					String.format("The encoding format you configured is not supported: [%s]", encoding), uce);
		} catch (Exception e) {
			throw DataXException.asDataXException(UnstructuredStorageReaderErrorCode.CONFIG_INVALID_EXCEPTION,
					String.format("Encoding configuration abnormal, please contact us: %s", e.getMessage()), e);
		}
	}
	...
	public static void validateFieldDelimiter(Configuration readerConfiguration) {
		...
		if(null == delimiterInStr){
			throw DataXException.asDataXException(UnstructuredStorageReaderErrorCode.REQUIRED_VALUE,
					String.format("You provided the configuration file incorrectly. [%s] is a required parameter.",
	...
	public static void validateColumn(Configuration readerConfiguration) {
		...
		if (null == columns || columns.size() == 0) {
			throw DataXException.asDataXException(UnstructuredStorageReaderErrorCode.REQUIRED_VALUE, "You need to specify Columns");
		}
		// handle ["*"]
		...
		if (null != columns && columns.size() != 0) {
			for (Configuration eachColumnConf : columns) {
				...
				if (null != columnIndex && columnIndex < 0) {
					throw DataXException.asDataXException(UnstructuredStorageReaderErrorCode.ILLEGAL_VALUE,
							String.format("Index has to be greater than or equal to 0, The index you configured is [%s]", columnIndex));
				}
			}
		}
	}
	...
	public static void validateCsvReaderConfig(Configuration readerConfiguration) {
		...
		if(StringUtils.isNotBlank(csvReaderConfig)){
			try{
				...
				LOG.info(String.format("WARN!!!!Ignore the CSVReaderConfig configuration!Error in configuration. The value can only be null or is a Map structure. The value you configured is: %s", csvReaderConfig));
			}
		}
	}
	...
	public static String getRegexPathParentPath(String regexPath){
		...
		if(parentPath.contains("*") || parentPath.contains("?")){
			throw DataXException.asDataXException(UnstructuredStorageReaderErrorCode.ILLEGAL_VALUE,
					String.format("Path: [%s] is invalid, currently only supports using wildcard * or? On the last level of directory.", regexPath));
		}
		return parentPath;
	}
	...
	public static void setCsvReaderConfig(CsvReader csvReader){
		if(null != UnstructuredStorageReaderUtil.csvReaderConfigMap && !UnstructuredStorageReaderUtil.csvReaderConfigMap.isEmpty()){
			try {
				...
				LOG.info(String.format("CSVReaderConfig is set successfully. After setting CSVReader :%s", JSON.toJSONString(csvReader)));
			} catch (Exception e) {
				LOG.info(String.format("WARN!!!!Ignore the CSVReaderConfig configuration!An exception occurred when configuring your CSVReaderConfig via BeanUtils.populate. The value you configured is: %s;Please check your configuration!CsvReader uses the default value [%s]",
						JSON.toJSONString(UnstructuredStorageReaderUtil.csvReaderConfigMap),JSON.toJSONString(csvReader)));
			}
		}else {
			//默认关闭安全模式, 放开10W字节的限制
			csvReader.setSafetySwitch(false);
			LOG.info(String.format("The default value of CSVReader is [%s] and the value of CSVReaderConfig is [%s].",JSON.toJSONString(csvReader),JSON.toJSONString(UnstructuredStorageReaderUtil.csvReaderConfigMap)));
		}
	}
}
